...
module.exports = {
    ...
    run: function (app, path, R2, io, fs, transporter, sha512, GetQuiz) {
        ...
        app.post('/V1/JoinLobby', (req, res) => {
            ...
            if (fs.existsSync(`./data/lobbies/${req.body.LobbyPin}.json`) === false) return res.status(400).send({"ErrCode":201,"ErrMsg":"Lobby does not exist"});
            fs.readFileSync(`./data/lobbies/${req.body.LobbyPin}.json`, "utf8", (err, data) => {
                console.log(req.body)
        ...
        app.post('/V1/JoinLobby/UsernameEnter', (req, res) => {
            if (!fs.existsSync(`../../../data/lobbies/${req.body.LobbyPin}.json`)) res.status(400).send({"ErrCode":201,"ErrMsg":"Lobby does not exist"});
            fs.readFileSync(`../../../data/lobbies/${req.body.LobbyPin}.json`, "utf8", (err, data) => {
                if (err) {console.error(err); res.status(400).send({"ErrCode":202,"ErrMsg":"Something went wrong..."});}
                let LobbyData = JSON.parse(data);
                //get lobby players count
